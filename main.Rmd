...
Skripti ottaa käyttöön tarvittavia kirjastoja ja funktioita ja lataa tarvittaessa rakennus- ja vaalidatan (http://www.avoindata.fi, http://tulospalvelu.vaalit.fi). *Jos tiedostot jo löytyvät, niitä ei haeta tai lasketa uudestaan.* Jos haluat niin tehdä, poista hakemistosta `data` tiedostot.

Datat tallennetaan hakemistoon `data`. Tilastointipostinumerot tallennetaan aliskiptissä `aanestysalue2postinumero.R`:ssä ja niissä on mukana alueiden nimet. Äänestystulokset tarvitaan äänestysalueittain (datassa ne ovat myös summattuna kunnittain jne)
...
# dataframe map.pono.aanestysalue jolla muunnoksen voi tehdä
...
Esimerkiksi alueelle 00980 summataan lähes kaikki äänet äänestysalueilta Vuosaari A, B ja E, ja vaihteleva osa äänistä alueista C, D, F, F, G ja H. Painot saadaan rakennusten määrien perustella (Ks. `aanestysalue2postinumero.R`). Painotuksen voi laskea myös toiseen suuntaan, jos haluaa aggregoida postinumeroittain annettua dataa äänestysalueille.

Huom: äänten kokonaissummat koko vaalipiirissä voi saada täsmäämään, mutta postinumeroalueen äänimäärä voi esimerkiksi ylittää asukasluvun (tai olla liian pieni). Kokonaissumankin osalta ne äänestysalueet, joita ei vastaa mikään postinumero, jäävät kokonaan pois.  
...
## Yhdistetään äänestystulos postinumeroalueiden Paavo-dataan
